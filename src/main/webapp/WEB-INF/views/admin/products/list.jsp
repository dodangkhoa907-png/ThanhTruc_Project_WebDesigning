        ...
        <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
            <%@ taglib prefix="fn" uri="jakarta.tags.functions" %>
                ...
                <style>
                    ...
                    .badge-ACTIVE {
                        background: rgba(42, 92, 56, .12);
                        color: var(--status-done)
                    }

                    .badge-INACTIVE {
                        background: rgba(138, 154, 138, .15);
                        color: var(--admin-text-light)
                    }
                    ...
                    .pf-modal textarea.form-control {
                        resize: vertical;
                        min-height: 88px
                    }
                    ...
                    .variant-row.marked-removed {
                        opacity: .45;
                        background: rgba(217, 83, 79, .05)
                    }

                    .variant-row .v-remove-btn {
                        background: none;
                        border: none;
                        color: var(--admin-red);
                        font-weight: 700;
                ...
                <c:if test="${not empty flashSuccess}">
                    <div class="pr-flash success">
                        <c:out value="${flashSuccess}" />
                    </div>
                </c:if>
                <c:if test="${not empty flashError}">
                    <div class="pr-flash error">
                ...
                <div class="card">
                    ...
                    <div class="pr-count">
                        ...
                        <button type="button" class="btn btn-primary" id="openCreateProductBtn"><i
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="pr-variants">
                                                        ...
                                                        <c:if test="${empty p.variants}"><span>Chưa có biến thể</span>
                                                        </c:if>
                                                    </div>
                                                </td>
                                                <td><span
                                                        class="badge badge-${p.active ? 'ACTIVE' : 'INACTIVE'}">${p.active
                                                        ? 'Đang bán' : 'Đã ẩn'}</span></td>
                                                <td>
                                                    <div class="pr-actions">
                                                        <button type="button" class="btn btn-outline js-edit-product"
                                                            data-id="${p.productId}">Sửa</button>
                                                        <form method="post" action="${ctx}/admin/san-pham/an-hien"
                                                            ...
                                                            <button type="submit"
                                                                class="btn ${p.active ? 'btn-danger' : 'btn-primary'}">${p.active
                                                                ? 'Ẩn' : 'Hiện'}</button>
                                                        </form>
                                                    </div>
                                                </td>
                ...
                <div class="pf-overlay" id="productFormOverlay" hidden>
                    <div class="pf-modal" role="dialog" aria-modal="true" aria-labelledby="productFormTitle">
                        <div class="pf-modal-head">
                            ...
                            <button type="button" class="pf-modal-close" id="productFormClose" aria-label="Đóng"><i
                            ...
                            <div class="pf-grid">
                                <div>
                                    <div class="card">
                                        ...
                                        <div class="form-group">
                                            <label>Mô tả</label>
                                            <textarea name="description" id="pfDescription" class="form-control"
                                    ...
                                    <div class="card">
                                        ...
                                        <button type="button" class="btn btn-outline" id="addVariantBtn"><i
                                                class="fa-solid fa-plus"></i> Thêm size</button>
                                        <p class="pf-hint">Cần ít nhất 1 size đang hoạt động. Xóa biến thể đã tồn tại
                                            chỉ ẩn đi (không mất dữ liệu đơn hàng cũ).</p>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div class="card">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <p class="pf-hint" id="pfImageHint">JPG/PNG/WEBP, tối đa 15MB — không bắt
                                                buộc.
                                                Nếu tên sản phẩm khớp trái cây có ảnh sẵn (cam/dưa hấu/thơm), trang chủ
                                                sẽ tự hiện ảnh đó.</p>
                                        </div>
                                    </div>

                                    <div class="pf-submit-row">
                                        <button type="submit" class="btn btn-primary" id="pfSubmitBtn"><i
                                                class="fa-solid fa-floppy-disk"></i> Thêm sản phẩm</button>
                                        <button type="button" class="btn btn-outline"
                                            id="productFormCancel">Hủy</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                    ...
                    (function () {
                        ...
                        function openProductForm(mode, data) {
                            ...
                            if (mode === 'edit') {
                                ...
                                pfImageHint.textContent = 'JPG/PNG/WEBP, tối đa 15MB — không bắt buộc. Bỏ trống để giữ ảnh hiện tại.';
                                ...
                                pfDescription.value = '';
                                pfCurrentImg.hidden = true;
                                pfImageHint.textContent = 'JPG/PNG/WEBP, tối đa 15MB — không bắt buộc. Nếu tên sản phẩm khớp trái cây có ảnh sẵn (cam/dưa hấu/thơm), trang chủ sẽ tự hiện ảnh đó.';
                                variantRowsEl.appendChild(buildVariantRow(0, 'M', null, false));
